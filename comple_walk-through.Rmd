...
We first want to visualize our data, see in general how things compare between species so we can better interpret our statistical results. A good way to do this is using boxplots. While it is possible to do boxplots from base functions, we can specify more using ggplot. You will need to install and upload the "ggplot2" package for this.
...
ggplot(raw, aes(x=SPP, y=T3.I, fill=SPP)) + geom_boxplot() + xlab("Species") + ylab("Number of incorrect choices\n") + ggtitle("Trial 3\n") + theme_classic() + scale_fill_manual(values = c("#9C964A", "#FAD77B")) + theme(text = element_text(size = 16)) + theme(legend.position = "none")
